...
class FastAcStarkTwoToneSpectroscopy(FastTwoToneSpectroscopyBase):
    ...
    def set_fixed_parameters(self, current=None, voltage=None, bandwidth_factor=10, **dev_params):
        ...
        super().set_fixed_parameters(voltage=voltage, current=current, detect_resonator=False,
                                     bandwidth_factor=bandwidth_factor, **dev_params)
    ...
    def _power_and_averages_setter(self, power):
        ...
        vna_parameters[0]["freq_limits"] = self._resonator_area

        self._mw_src[0].set_output_state("OFF")
        if vna_parameters[0]["freq_limits"][0] != vna_parameters[0]["freq_limits"][1]:
            # print("\rDetecting a resonator within provided frequency range of the VNA %s\
            #        "%(str(vna_parameters["freq_limits"])), flush=True, end="")

            res_result = self._detect_resonator(vna_parameters[0], plot=False)
            ...
            res_freq = vna_parameters[0]["freq_limits"][0]

        self._mw_src[0].set_output_state("ON")
        vna_parameters[0]["freq_limits"] = (res_freq, res_freq)

        self._vna[0].set_parameters(vna_parameters[0])
