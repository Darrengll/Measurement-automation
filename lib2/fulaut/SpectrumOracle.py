...
class SpectrumOracle:
    ...
    def launch(self):
        self._search_very_coarse()
        self._search_coarse()
        self._search_fine_multiline()
        return array(self._fine_opt_params)

    def _search_fine_multiline(self):
        fine_period_grid = slice(self._opt_params_coarse[0],
                                 self._opt_params_coarse[0] + 0.1,
                                 1)
        fine_sws_grid = slice(self._opt_params_coarse[1],
                              self._opt_params_coarse[1] + 0.1,
                              1)
        fine_freq_grid = slice(self._coarse_frequency,
                               self._coarse_frequency + 0.41,
                               0.4 / 20)
        fine_d_grid = slice(self._opt_params_coarse[3],
                            self._opt_params_coarse[3] + .1,
                            1)
        ...
        self._iterations = reduce(multiply,
                                  [int((slice.stop - slice.start) / slice.step) + 1 for slice in
                                   self._fine_slices])
        ...
        self._fine_chosen_points = self._cost_function_fine_fast(best_solution, self._y_scan_area_size,
                                                      self._points, True)[1]
        all_chosen = concatenate([points for points in self._fine_chosen_points if points.size > 0])
        result = minimize(self._cost_function_fine_fast, best_solution,
                          args=(self._y_scan_area_size, all_chosen), method="Nelder-Mead")
        best_solution = result.x

        self._fine_chosen_points = self._cost_function_fine_fast(best_solution,
                                                                 self._y_scan_area_size,
                                                                 self._points,
                                                                 True)[1]
        ...
        if self._plot:
            self._plot_fine_multiline()
            plt.gcf().set_size_inches(15, 5)
            plt.legend()



    def _search_coarse(self):
        ...
        slices = (period_slice, sws_slice, freq_slice, d_slice)
        self._iterations = reduce(multiply,
                                  [int((slice.stop - slice.start) / slice.step) + 1 for slice in
                                   slices])
        ...
        result = minimize(self._cost_function_coarse, opt_params_coarse,
                          args=(self._y_scan_area_size, self._coarse_chosen_points), method="Nelder-Mead")
    ...
    def _search_very_coarse(self):
        ...
        if self._plot:
            self._plot_rating_curves(10,
                                     self._very_coarse_nops,
                                     self._very_coarse_candidates,
                                     "Very coarse search",
                                     0.5)

    def _plot_fine_multiline(self):
        for idx, branch in enumerate(self._fine_chosen_points):
            if branch.size > 0:
                plt.plot(branch[:, 0], branch[:, 1], "C%d." % idx)

        opt_params = self._fine_opt_params.copy()
        opt_params[2] /= 1e9
        opt_params[-1] /= 1e9
        ...
        for i in range(0, 4):
            plt.plot(self._parameter_values,
                     self._qubit_spectrum(self._parameter_values,
                                          *opt_params[:-1]) -
                     i * opt_params[-1]/2, color="C%d" % i)
        plt.xlim(self._parameter_values[0], self._parameter_values[-1])
        plt.ylim(self._frequencies[0], self._frequencies[-1])


    def _plot_rating_curves(self, N, nops, candidates, label, lw = 0.5):
        N = min(N, len(candidates))
        ptp_rating = max(1e-3, ptp(1 / array(nops[:N])))
        min_rating = 1 / max(nops[:N])
        for idx, opt_params, nop in zip(range(N), candidates[:N],
                                        nops[:N]):
            rating = 1 / nop
            plt.plot(self._parameter_values,
                     self._qubit_spectrum(self._parameter_values,
                                          *opt_params), ":",
                     color=(1 - (rating - min_rating) / ptp_rating, 0,
                            (rating - min_rating) / ptp_rating), lw=lw,
                     label=label if idx == 0 else None, alpha = 0.5)

    def _plot_data(self):
        plt.plot(self._points[:, 0], self._points[:, 1], ".", markersize=2, color="black",
    ...
    def _extract_data(self):
        ...
        for point in points:
            very_close_y_indices = \
                where(abs(self._points[:, 1] - point[1]) < self._freq_resolution/2)[0]
        ...
        if self._plot:
            ...
            cb = plt.colorbar()
            cb.ax.plot([0, 1], [self._threshold] * 2, color="red")
            plt.gcf().set_size_inches(15, 5)
    ...
    def _cost_function_coarse(self, params, y_scan_area_size, points,
        ...
        d = params[3]
        if len(chosen_points) < len(set(self._points[:, 0])) * .5 or d > 0.9:
    ...
    def visualize(self):
        ...
        data = {'extracted points': self._points, 'parameters': self._parameter_values,
                'fine opt parameters': self._fine_opt_params,
                'opt params coarse': self._opt_params_coarse}
        self._data = data
