...
class KeysightM3202A:
    ...
    def __init__(self, slot, chassis=0, allow_unmatched_waveforms = True):
        ...
        # trigger_ext_sources can be from the front panel, can be also from PXI_n triggering bus
    ...
    def set_trigger(self, trigger_string: str = "CONT", channel: int = -1):
        ...
        for channel in channels_to_config:
            if trigger_string == "EXT":  # front panel
                ...
                ret = self.module.AWGtriggerExternalConfig(channel - 1, self.trigger_ext_sources[channel - 1],
                                                           # front panel only
        ...
        for chan in channels_to_config:
            if self.trigger_output:  # enable trigger for the first channel from group
                ...
                self.module.AWGqueueMarkerConfig(chan - 1, SD_MarkerModes.EVERY_CYCLE,
                                                 trgPXImask, trgIOmask, 1, syncMode=self.sync_mode,
    ...
    def output_arbitrary_waveform(self, waveform, frequency, channel, asynchronous=False):
        ...
        self._load_waveform_to_channel(waveform, frequency, channel)
        # starting operation
        self._start_AWG(channel)
    ...
    def setup_modulation_amp(self, channel, deviation_gain):
        ...
        self.module.modulationAmplitudeConfig(channel - 1, SD_ModulationTypes.AOU_MOD_AM,
                                              deviation_gain)

    def _load_waveform_to_channel(self, waveform, frequency, channel):
        if np.max(np.abs(waveform)) >= 1.5:
            raise Exception("signal maximal amplitude is exceeding AWG range: (-1.5 ; 1.5) volts")

        # number of points
        if (round(frequency * (len(waveform))) > 1e9):
    ...
    def _start_AWG(self, channel):
        ...
        if ((not self.synchronized_channels) or (channel not in self.synchronized_channels)):
            ...
            raise NotImplementedError(
                "Check channel number conditions on argument provided: channel={}".format(channel))
    ...
    def _load_dependent_channels(self):
        ...
        for source_channel_idx, (source_chan, dependent_chan) in enumerate(
                zip(self._source_channels_group, self._dependent_channels_group)):
