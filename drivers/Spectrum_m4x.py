...
class SPCM:
    ...
    def __init__(self, path):
        ...
        self._segment_size_optimal = None
    ...
    def set_parameters(self, pars_dict):
        ...
        if ("channels" in pars_dict and
            ...
            self._segment_size_optimal = int(pars_dict["dur_seg"] * self.get_sample_rate())
            self._segment_size = self._segment_size_optimal + 32 - self._segment_size_optimal % 32
            self._bufsize = pars_dict["n_seg"] * self._segment_size * 4 * len(pars_dict["channels"])
    ...
    def __def_simp_transfer(self, buffer, notifysize=0, offset=0):
        ...
        return spcm_dwDefTransfer_i64(self.hCard, SPCM_BUF_DATA, SPCM_DIR_CARDTOPC, int32(notifysize), byref(buffer), int64(offset), int64(buffer.__len__()))
    ...
    def setup_block_avg_STD(self, memsize, segmentsize, posttrigger, averages):
        ...
        self.__write_to_reg_32(SPC_POSTTRIGGER, posttrigger)  # Post trigger memory size (pretrigger  = segmentsize - posttrigger)
    ...
    def setup_channel(self, channelnum, amplitude):
        ...
        self.__write_to_reg_32(SPC_AMP0 + 100 * channelnum, amplitude) # Set the input amplitude (valid values: 200, 500, 1000, 2500 mV)
    ...
    def setup_channels(self, channels, amplitude):
        ...
        for chan in channels:
            mask |= CHANNEL0 << chan
            self.__write_to_reg_32(SPC_AMP0 + 100 * chan, amplitude) # Set the input amplitude (valid values: 200, 500, 1000, 2500 mV)
    ...
    def setup_ext0_trigger(self):
        ...
        self.__write_to_reg_32(SPC_TRIG_EXT0_MODE, self._trigger_mode) # trigger on the rising edge (voltage crosses 0-level barrier)
    ...
    def obtain_data(self, bufsize):
        ...
        if res is not 0:
            print("Error: %d" % res)
            return None
        self.__write_to_reg_32(SPC_M2CMD, M2CMD_DATA_STARTDMA | M2CMD_DATA_WAITDMA) # Start the transfer and wait till it's completed
    ...
    def setup_averaging_mode(self, channels, ampl, num_segments, segment_size, pretrigger, num_averages):
...
#         plt.ylim(-ymax * 0.1, 1.1 * ymax)
    ...
    def plot_signal(channels, data, ampl, time_from, time_until, samplerate, num_averages=1):
        ...
        freq = samplerate / 1e6
        start_x = int(time_from * freq)
        end_x = int(time_until * freq)
        
        xdata = np.arange(0, len(data) / N) / freq
    ...
    def show_averaged_channel(self, channels, ampl, time_from, time_until, num_averages=10000):
        """
        ...
        """
        freq = self.get_sample_rate() / 1e6  # MHz
        segment_size = int(time_until * freq)
        segment_size += 32 - segment_size % 32  # completing requested signal length to the multiple of 32
        num_segments = 1
        data = self.measure_averaging_mode(channels, ampl, num_segments, segment_size, 32, num_averages)
    ...
    def show_averaged_spectrum(self, channels, ampl, n_seg, segment_time_length, pretrigger, freq_from, freq_until, num_averages = 10000):
