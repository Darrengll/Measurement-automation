...
class RabiTestCase(KtQetExperimentTestCase):
    ...
    def setUp(self):
        ...
        self.experiment = Rabi(self.qubit)
    ...
    def test_run(self):
        ...
        # Try again, but this time we expect pi2 waveform exception
        try:
            self.experiment.run('')
            self.fail()
        except ExperimentNotConfiguredException as e:
            self.assertEqual(str(e), 'Ramp waveforms not configured.')


        # Configure the waveforms
        test_rampup_wave_i = SD1.SD_Wave()
        test_rampup_wave_i.newFromFile(os.path.abspath('..\\..\\include\\waveforms\\') + '\\' + 'RampUp.csv')
        test_rampup_wave_q = SD1.SD_Wave()
        test_rampup_wave_q.newFromFile(os.path.abspath('..\\..\\include\\waveforms\\') + '\\' + 'RampUp.csv')
        test_rampdown_wave_i = SD1.SD_Wave()
        test_rampdown_wave_i.newFromFile(os.path.abspath('..\\..\\include\\waveforms\\') + '\\' + 'RampDown.csv')
        test_rampdown_wave_q = SD1.SD_Wave()
        test_rampdown_wave_q.newFromFile(os.path.abspath('..\\..\\include\\waveforms\\') + '\\' + 'RampDown.csv')

        self.experiment._configure_rabi_waveforms(test_rampup_wave_i, test_rampup_wave_q, test_rampdown_wave_i,
                                                test_rampdown_wave_q)
...
def suite():
    ...
    suite.addTests(unittest.TestLoader().loadTestsFromTestCase(RabiTestCase))
    return suite
