...
class EnergyRelaxation(KtQetExperiment):
    """
    EnergyRelaxation is an extension of KtQetExperiment and is designed to demonstrate 
    T1 measurements on qubits. A pi pulse is sent out to set the qubit in its excited state,
    and a variable amount of time is waited before a readout pulse. This helps determine
    how long the qubit can maintain a state.
    ...
    def run(self, hvi_file_path):
        ...
        self.qubit.awg_control.AWGqueueWaveform(self.qubit.awg_control_channel_i, pi_waveform_id_i,
                                                SD1.SD_TriggerModes.SWHVITRIG_CYCLE, 
                                                delay, self.INFINITE_CYCLES, prescalar)
